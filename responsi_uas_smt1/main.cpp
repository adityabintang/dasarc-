...
void hitungBonusPerumahan(int masaKerja, int &bonusPerumahan) {
    bonusPerumahan = (masaKerja > 10) ? 3000000 : 0;
}

void hitungKelebihanSKS(int sks, string jafa, int &totalKelebihanSKS) {
    if (sks <= 12) {
        totalKelebihanSKS = 0;
        return;
    }
    ...
    totalKelebihanSKS = kelebihanSKS * tarif;
}

void hitungLembur(int lembur, string golongan, int masaKerja, int &totalLembur) {
    if (masaKerja < 1 || lembur <= 0) {
        totalLembur = 0;
        return;
    }
    int tarif = 0;
...
void prosesGajian(vector<Pegawai>& pegawaiList, vector<Dosen>& dosenList) {
    ...
    if (pilihan == "Pegawai") {
        for (auto& p : pegawaiList) {
            int bonus;
            hitungBonusPerumahan(p.masaKerja, bonus);
            int lembur;
            hitungLembur(p.lembur, p.golongan, p.masaKerja, lembur);
            p.gajiDiterima = p.gajiPokok + bonus + lembur;
        }
    } else if (pilihan == "Dosen") {
        for (auto& d : dosenList) {
            int bonus;
            hitungBonusPerumahan(d.masaKerja, bonus);
            int kelebihanSKS;
            hitungKelebihanSKS(d.sks, d.jafa, kelebihanSKS);
...
void tampilkanLaporan(const vector<Pegawai>& pegawaiList, const vector<Dosen>& dosenList) {
    ...
    for (const auto& d : dosenList) {
        ...
        totalGapok += d.gajiPokok;
        totalLembur += kelebihanSKS;
        totalGaji += d.gajiDiterima;
    }

    cout << "=====================================================================================\n";
    cout << "TOTAL " << setw(46) << totalGapok << " " << setw(12) << totalLembur << " " << setw(12) << totalGaji << "\n";
}
